...
class EmailTriageAction(Action):
    ...
    The agent must classify the email along three orthogonal axes:
      ...
      route     -- which queue or team should receive it?
    ...
    For phishing or social engineering emails, the correct category is 'security'
...
class EmailTriageObservation(Observation):
    ...
    # Feedback about the immediately preceding action (None on first observation)
    ...
    emails_remaining: int = Field(
        default=0,
        description="Emails left to process after this one (0 means last email)",
    )

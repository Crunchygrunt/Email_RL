...
class EmailTriageEnv(
    ...
    def _parse_result(self, payload: Dict) -> StepResult[EmailTriageObservation]:
        ...
        observation = EmailTriageObservation(
            ...
            emails_remaining = obs_data.get("emails_remaining", 0),
            current_streak   = obs_data.get("current_streak", 0),
            # Ground truth for client-side graders (BUGFIX: these six fields
            # were previously omitted here, so every EmailTriageObservation
            # built from a WebSocket response silently defaulted to
            # true_priority=None / true_category=None / true_route=None /
            # is_business_critical=False / is_phishing=False /
            # linked_incident=False regardless of what the server actually
            # sent -- see WAREHOUSE.md "Known findings" for how this was
            # found and confirmed).
